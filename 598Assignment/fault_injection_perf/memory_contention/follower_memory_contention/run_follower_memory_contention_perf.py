...
print("Fault injection: follower memory contention")
...
command_inject_memory_contention = ["bash", "generate_memory_contention_fault.sh", str(mem_val)]
process_inject_memory_contention = subprocess.Popen(command_inject_memory_contention, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
out, err = process_inject_memory_contention.communicate()
if err:
    print("ERR: ", err)
...
for i in range(2, 22, 2):
    ...
    print("memory contention - follower with id 3 ... ")
    command_follower_memory_contention = ["bash", "follower_memory_contention.sh", str(process2_pid)]
    process_follower_memory_contention = subprocess.Popen(command_follower_memory_contention, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    out, err = process_follower_memory_contention.communicate()
    if err:
        print("ERR: ", err)
